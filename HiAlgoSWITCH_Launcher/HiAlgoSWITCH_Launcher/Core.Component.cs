...
namespace HiAlgoSWITCH_Launcher
{
	class Component
	{
		...
		{
			if (!File.Exists(Global.iniFileName) || Global.iniFirstRun == "Enabled" || File.ReadAllLines(Global.iniFileName).Length < 6 || new FileInfo(Global.iniFileName).Length < 109)
			{
		...
		public static void CheckLogCleaner()
		{
			if (Global.iniClearHialgoLogs == "Enabled")
			{
				try
				{
